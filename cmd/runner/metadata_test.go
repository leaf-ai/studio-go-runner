...
func waitForMetaDataRun(ctx context.Context, qName string, queueType string, r *request.Request, prometheusPort int) (err kv.Error) {
...
func validateMultiPassMetaData(ctx context.Context, experiment *ExperData, rpts []*runnerReports.Report) (err kv.Error) {
	...
	if errCount := checkMDCount(ctx, experiment); errCount != nil {
		...
		logger.Warn("failed check of metadata", "error", errCount, "stack", stack.Trace().TrimRuntime)
