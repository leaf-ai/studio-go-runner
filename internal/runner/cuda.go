...
// GPUTrack is used to track usage of GPU cards and any errors generated by the cards
...
type GPUTrack struct {
	...
	EccFailure *errors.Error       // Any Ecc failure related error messages, nil if no errors encountered
...
// when started as a go function check the devices for ECC and other errors marking
// failed GPUs
//
...
func (allocator *gpuTracker) AllocGPU(maxGPU uint, maxGPUMem uint64, unitsOfAllocation []uint) (alloc GPUAllocations, err errors.Error) {
	...
	if len(combinations) == 0 {
		return nil, errors.New("insufficient GPU devices").With("stack", stack.Trace().TrimRuntime())
	}
	...
	if len(matched.cards) == 0 {
		return nil, errors.New("insufficient GPU slots").With("stack", stack.Trace().TrimRuntime())
	}
