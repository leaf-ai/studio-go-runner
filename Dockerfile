...
CMD /bin/bash -c '(go get github.com/karlmutch/duat && go get github.com/karlmutch/enumer && dep ensure && go generate ./internal/types && go build -o /project/bin/build -tags NO_CUDA build.go && /project/bin/build -r -dirs internal && /project/bin/build -dirs cmd/runner) 2>&1 | tee $RUNNER_BUILD_LOG'
