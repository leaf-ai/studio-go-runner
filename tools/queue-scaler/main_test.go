...
// go test -coverpkg="." -c -o bin/queue-scaler tools/queue-scaler/*.go
//
// Then the resulting /bin/queue-scaler-coverage binary is run as though it were a traditional
...
//
// ./bin/queue-scaler-coverage -test.run "^TestRunMain$" -test.coverprofile=system.out
//
